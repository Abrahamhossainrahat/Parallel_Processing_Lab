...
/* 
For compile : !nvcc -arch=sm_75 search_phonebook.cu -o search_phonebook
For run : !nvcc -arch=sm_75 search_phonebook.cu -o search_phonebook
For run(another thread) : !time ./search_phonebook JAHAN 2 > output1.txt
